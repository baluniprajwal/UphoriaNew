...
const Artists: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      visualizerTweenRef.current = gsap.to(".sound-bar-active", {
        ...
        backgroundColor: () =>
          gsap.utils.random(["#FF007F", "#00F0FF", "#FFD60A"]),
        duration: 0.08,
        repeat: -1,
        yoyo: true,
        ease: "power1.inOut",
      ...
      titlePulseRef.current = gsap.to(".main-title", {
        textShadow:
          "0px 0px 20px rgba(255, 0, 127, 0.8), 0px 0px 40px rgba(0, 240, 255, 0.6)",
        scale: 1.05,
        duration: 0.4,
        repeat: -1,
        yoyo: true,
  ...
  return (
    <section
      id="artists"
      ref={containerRef}
      className="py-24 md:py-32 relative overflow-hidden bg-[#111] border-y-4 border-black min-h-screen flex flex-col justify-center perspective-1000"
    >
      {}
      {}
      ...
      <div className="vibe-content w-full h-full relative">
        ...
        <div className="absolute top-10 md:top-20 left-0 w-[200%] md:w-full opacity-20 pointer-events-none select-none overflow-hidden whitespace-nowrap z-0">
          <div
            ref={marqueeRef}
            className="font-display font-black text-[10rem] md:text-[15rem] leading-none text-transparent text-outline-white tracking-tighter"
          >
        ...
        <div className="bg-widget absolute -bottom-20 -right-20 md:bottom-10 md:right-10 opacity-80 z-0 pointer-events-none">
          <div
            ref={vinylRef}
            className="w-64 h-64 md:w-96 md:h-96 bg-black rounded-full border-4 border-uphoria-pink flex items-center justify-center relative shadow-2xl"
          >
            ...
            <div className="w-24 h-24 md:w-32 md:h-32 bg-uphoria-yellow rounded-full flex items-center justify-center border-4 border-black">
              <Music
                className={`text-black w-10 h-10 md:w-16 md:h-16 ${
        ...
        <div className="bg-widget absolute top-40 left-10 md:left-20 z-0 pointer-events-none opacity-90">
          <div className="flex items-end gap-2 h-32">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className={`w-4 md:w-6 border-2 border-black transition-all ${
                  isPlaying
                    ? "bg-uphoria-cyan h-full animate-pulse"
                    : "bg-gray-700 h-10"
                }`}
              ></div>
            ))}
          </div>
          <div className="text-uphoria-cyan font-display font-bold mt-2 rotate-1 tracking-widest">
            AUDIO VISUALIZER
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col items-center mb-16 md:mb-20 text-center">
            ...
            <h2 className="main-title relative z-10 font-display text-5xl md:text-7xl lg:text-8xl font-black text-white uppercase tracking-tighter mix-blend-difference transition-all duration-300">
              {"LINEUP".split("").map((char, i) => (
                <span
                  key={i}
                  className="artist-title-char inline-block hover:text-uphoria-pink transition-colors cursor-default hover:-translate-y-4 hover:rotate-6 duration-300"
                >
                  {char}
                </span>
              ))}
          ...
          <div className="artists-grid grid grid-cols-1 md:grid-cols-3 gap-12 max-w-7xl mx-auto items-start">
            {artists.map((artist) => {
              ...
              return (
                <div
                  key={artist.id}
                  className={`artist-card group relative ${artist.rotate} hover:z-20 transition-all duration-500`}
                >
                  {}
                  <div
                  ...
                  {}
                  <div
                    className={`relative p-3 pb-8 border-4 hard-shadow transition-all duration-300 transform group-hover:-translate-y-2 ${
                      !isPlaying
                        ? "bg-white border-black group-hover:shadow-[8px_8px_0px_#FF007F]"
                        : ""
                    }`}
                    style={glowStyle}
                  >
                    {}
                    <div
                      className={`relative aspect-[3/4] border-2 overflow-hidden bg-black mb-4 transition-colors duration-300 ${
                        isPlaying ? "border-gray-500" : "border-black"
                      }`}
                    >
                      ...
                      <img
                        src={artist.image}
                        alt={artist.name}
                        className={`w-full h-full object-cover transition-all duration-500 transform group-hover:scale-110 group-hover:rotate-1 ${
                          isPlaying
                            ? "grayscale-0 contrast-125 saturate-150"
                      ...
                      {}
                      <div
                        className={`absolute top-2 right-2 flex items-center gap-1 bg-black/80 px-2 py-1 rounded-full border border-white/20 z-20 transition-opacity duration-300 ${
                          isPlaying
                            ? "opacity-100 shadow-[0_0_10px_red]"
                            : "opacity-0"
                        }`}
                      >
                    ...
                    <div className="px-2">
                      <h3
                        className={`font-display text-3xl md:text-4xl font-black uppercase mb-1 leading-none tracking-tight group-hover:text-uphoria-purple transition-colors duration-300 ${
                      ...
                      <div
                        className={`flex flex-col gap-2 mt-4 pt-4 border-t-2 transition-colors duration-300 ${
                          isPlaying ? "border-white/20" : "border-black/10"
                        }`}
                      >
                        <div
                          className={`flex items-center gap-2 font-sans font-bold transition-colors ${
                            isPlaying
                  ...
                  {!isPlaying && (
                    <div
                      className={`absolute -inset-2 ${artist.color} -z-10 border-2 border-black hidden group-hover:block transition-all duration-300`}
                    ></div>
                  )}
                </div>
              );
          ...
          <div className="mt-24 flex flex-col items-center justify-center gap-6 relative z-50">
            <button
              onClick={toggleMusic}
              className={`group flex items-center gap-3 px-8 py-3 rounded-full border-4 border-black transition-all duration-300 hard-shadow active:translate-y-1 active:shadow-none hover:scale-105 ${
                isPlaying
                  ? "bg-uphoria-pink text-white animate-bounce shadow-[0_0_20px_#FF007F]"
                  : "bg-white text-black"
              }`}
            >
              {isPlaying ? (
      ...
      <div
        className={`absolute bottom-0 left-0 w-full h-40 md:h-72 pointer-events-none flex items-end justify-center gap-[2px] md:gap-1 px-4 z-30 transition-opacity duration-500 ${
